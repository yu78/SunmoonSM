<?php
	...
	if($id_check >= 1){
		echo "<script>alert('このIDは、すでに登録されています。'); history.back();</script>";
	}
	else if(!(strlen($student_idx)==10)) {
		echo "<script>alert('学籍番号は10桁です。'); history.back();</script>";
	}
	else if($s_no_check >= 1) {
		echo "<script>alert('この学籍番号は すでに登録されています。'); history.back();</script>";
	}
	else{
...
?>
<meta charset="utf-8" />
<script type="text/javascript">alert('加入が完了しました。'); self.close(); </script>
